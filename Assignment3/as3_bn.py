...
class Classifier():
  ...
  def _initialization(self,shape):
    w=tf.random.normal([shape[0],shape[1]],0,1e-4 ,tf.float64)
    b=tf.zeros([shape[0],1],tf.float64)
    beta=tf.zeros([shape[0],1],tf.float64)
    gamma=tf.ones([shape[0],1],tf.float64)
  ...
  def evaluateClassifier(self,X):
    ...
    if self.batch_normalization==False:
      ...
      for i in range(self.k-1):
        ...
        variance.append(var)
        s_hat = (s - mu) / np.sqrt(var + np.finfo(np.float64).eps)
        S_hat.append(s_hat)
  ...
  def BatchNormBackPass(self,G_batch,S,mu,var):
    sigma_1 = np.power(var + tf.ones([G_batch.shape[0],1],tf.float64)*np.finfo(float).eps, -0.5)
    sigma_2 = np.power(var + tf.ones([G_batch.shape[0],1],tf.float64)*np.finfo(float).eps, -1.5)
  ...
  def cyclic_Minibatch_gd(self,train_X,train_Y,train_y,epochs,batch_size,eta_min,eta_max,n_s,lmd,shuffle):
    ...
    if self.batch_normalization==False:
      ...
      with open("bn_train_cost.txt","w") as train_cost_file:
        train_costs=str(train_costs)
        train_cost_file.write(train_costs)
      with open("bn_validation_cost.txt","w") as validation_cost_file:
        validation_costs=str(validation_costs)
        validation_cost_file.write(validation_costs)
...
classifier=Classifier(train_X,train_Y,train_y,k=3,batch_normalization=True)
