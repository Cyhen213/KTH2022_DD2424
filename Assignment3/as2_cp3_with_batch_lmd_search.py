...
class Classifier():
  ...
  def test_evaluateClassifier(self,X):
    ...
    for i in range(self.k-1):
      s=tf.matmul(self.best_W[i],s)+tf.matmul(self.best_b[i],n_b)
      S.append(s)
      ...
      S_hat.append(s_hat)
      s_tilde=np.multiply(tf.matmul(self.best_gamma[i],n_b),s_hat)+tf.matmul(self.best_beta[i],n_b)
      if self.activation[i]=="relu":
        s=tf.nn.relu(s_tilde)
      H.append(s)
    p=tf.matmul(self.best_W[self.k-1],s)+tf.matmul(self.best_b[self.k-1],n_b)
    p=tf.nn.softmax(p,0)
    H.append(p)
  ...
  def test_accuracy(self,X,y):
    if self.batch_normalization==True:
      _,_,_,_,H=self.test_evaluateClassifier(X)
      P=H[-1]
...
for i in range(8):
  ...
  lmd=math.pow(10,l)
  print(lmd)
  classifier.cyclic_Minibatch_gd(train_X,train_Y,train_y,epochs=epochs,batch_size=batch_size,eta_min=eta_min,eta_max=eta_max,n_s=n_s,lmd=lmd,shuffle=True)
  acc=classifier.test_accuracy(Xte,yte)
  print(acc)
