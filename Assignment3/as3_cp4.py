...
class Classifier():
  ...
  def evaluateClassifier(self,X):
    ...
    if self.batch_normalization==False:
      ...
      for i in range(self.k-1):
        ...
        S_hat.append(s_hat)
        s_tilde=np.multiply(tf.matmul(self.gamma[i],n_b),s)+tf.matmul(self.beta[i],n_b)
       # S_tilde.append(s_tilde)
        if self.activation[i]=="relu":
  ...
  def generate_eta(eta_min,eta_max,n_s,t):
    ...
    current_cycle = np.floor(t / (2 * n_s))
      ...
      for i in tqdm(range(epochs)):
        ...
        for j in range(n):
          ...
          for k in range(self.k):
            Wstar=np.copy(self.W[k])
            bstar=np.copy(self.b[k])
            Wstar=Wstar-eta*J_W[k]
            bstar=bstar-eta*J_b[k]
        for k in range(self.k):
          self.W[k]=Wstar
          self.b[k]=bstar
        if shuffle==True:
...
import numpy as np  
np.save('/cfs/klemming/home/y/yuchenga/DD2424/assignment3/W.npy', self.W)
np.save('/cfs/klemming/home/y/yuchenga/DD2424/assignment3/b.npy', self.b)
